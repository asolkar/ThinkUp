 ...
 *
 * The first PHP Library to support OAuth for StatusNet's REST API.
 */
...
class StatusNetOAuth {
  ...
  public $http_info;
  /* Set the useragnet. */
  public $useragent = 'StatusNetOAuth v0.2.0-beta2';
  ...
  /**
   * construct StatusNetOAuth object
   */
  ...
  /**
   * Get a request_token from StatusNet
   *
   ...
   * @returns a string
   */
  function getAuthorizeURL($token, $sign_in_with_statusnet = TRUE) {
    if (is_array($token)) {
      $token = $token['oauth_token'];
    }
    if (empty($sign_in_with_statusnet)) {

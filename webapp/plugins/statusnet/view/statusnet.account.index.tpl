...
{insert name="help_link" id='statusnet'}
<h2>StatusNet Plugin </h2>

    <div class="">
    <p>The StatusNet plugin captures and displays tweets, replies, mentions, retweets, friends, followers, favorites, links, and photos.</p>
...
<a href="{$oauthorize_link}" class="linkbutton emphasized">Add a StatusNet account</a>
<br /><br />
</div>
{/if}
{if count($owner_instances) > 0 }
<br>
<h2 class="subhead">StatusNet Accounts</h2>
    {foreach from=$owner_instances key=iid item=i name=foo}
        ...
        <div class="grid_8 right">
            <span id="delete{$i->id}"><form method="post" action="{$site_root_path}account/?p=statusnet">
            <input type="hidden" name="instance_id" value="{$i->id}">
            {insert name="csrf_token"}<input
            onClick="return confirm('Do you really want to delete this StatusNet account?');"
...
<!--<p>Alternately, add a public StatusNet username for ThinkUp capture data about:</p>
<form method="get" action="index.php"><input type="hidden" name="p" value="statusnet"><input name="statusnet_username" /> <input type="submit" value="Add this Public User to ThinkUp"></form>-->

{if $user_is_admin}
{include file="_plugin.showhider.tpl"}
{include file="_usermessage.tpl" field="setup"}

<p style="padding:5px">To set up the StatusNet plugin:</p>
<ol style="margin-left:40px"><li><a href="http://identi.ca/settings/oauthapps/new" target="_blank" style="text-decoration: underline;">Create a new application on StatusNet for ThinkUp</a>.</li>
<li>
    ...
    Callback URL:
    <small>
      <code style="font-family:Courier;" id="clippy_2988">http{if $smarty.server.HTTPS}s{/if}://{$smarty.server.SERVER_NAME}{$site_root_path}plugins/statusnet/auth.php</code>
    </small>
    <object classid="clsid:d27cdb6e-ae6d-11cf-96b8-444553540000"
              width="100"
              height="14"
...
<li>Enter the StatusNet-provided consumer key and secret here.</li></ol>
{/if}

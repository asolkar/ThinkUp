...
/**
 *
 * ThinkUp/webapp/plugins/statusnet/model/class.StatusNetCrawler.php
 *
 ...
 * StatusNet Crawler
 *
 * Retrieves tweets, replies, users, and following relationships from StatusNet.com
 *
 ...
 *
 */
class StatusNetCrawler {
    /**
     * @var Instance ThinkUp network account instance, ie, @thinkupapp on StatusNet
     */
    var $instance;
    /**
     ...
     */
    var $statusnet_options;

    /**
     * Constructor
     * @param Instance $instance
     * @param CrawlerStatusNetAPIAccessorOAuth $api
     * @return StatusNetCrawler
     */
    public function __construct($instance, $api) {
        ...
        $this->statusnet_options = $plugin_option_dao->getOptionsHash('statusnet');
    }
    /**
    ...
    public function fetchInstanceUserInfo() {
        if ($this->api->available && $this->api->available_api_calls_for_crawler > 0) {
            ...
            list($cURL_status, $statusnet_data) = $this->api->apiRequest($owner_profile);
            if ($cURL_status == 200) {
                $users = $this->api->parseXML($statusnet_data);
                ...
                if (isset($this->user)) {
                    ...
                    if (isset($this->user->follower_count) && $this->user->follower_count>0) {
                        ...
                        $fcount_dao->insert($this->user->user_id, 'statusnet',
                        $this->user->follower_count);
                    }
                    $this->logger->logUserSuccess("Successfully fetched ".$this->user->username.
                    "'s details from StatusNet. StatusNet's tweet count is ". $this->user->post_count,
                    __METHOD__.','.__LINE__);
                } else {
                    $this->logger->logUserError("StatusNet didn't return information for "
                    .$this->user->username, __METHOD__.','.__LINE__);
                    $this->logger->logError("StatusNet payload: ".$statusnet_data, __METHOD__.','.__LINE__);
                }
            }
        }
    }
    /**
     * Store StatusNet search results in the database.
    ...
    public function fetchSearchResults($term) {
        ...
        if (isset($this->user)) {
            ...
            while ($continue_fetching) {
                ...
                try {
                    list($cURL_status, $statusnet_data) = $this->api->apiRequest($search_results, null, false);
                } catch (APICallLimitExceededException $e) {
                    break;
                }
                if ($cURL_status == 200) {
                    $tweets = $this->api->parseJSON($statusnet_data);
                    $post_dao = DAOFactory::getDAO('PostDAO');
                    $count = 0;
                    foreach ($tweets as $tweet) {
                        $tweet['network'] = 'statusnet';
                        $inserted_post_key = $post_dao->addPost($tweet);
                        if ($inserted_post_key !== false) {
                            $count = $count + 1;
                            URLProcessor::processPostURLs($tweet['post_text'], $tweet['post_id'], 'statusnet',
                            $this->logger);
    ...
    private function processDeletedTweets() {
        if ($this->api->available && $this->api->available_api_calls_for_crawler > 0) {
            ...
            list($cURL_status, $statusnet_data) =
            ...
            if ($cURL_status == 200) {
                $count = 0;
                $tweets = $this->api->parseXML($statusnet_data);
                $tweets_array = array();
                $last_id = 0;
                foreach($tweets as $tweet) {
                ...
                $post_dao = DAOFactory::getDAO('PostDAO');
                $db_posts = $post_dao->getAllPosts($this->instance->network_user_id, 'statusnet',
                ...
                foreach($db_posts as $post) {
                    if (!isset($tweets_array[ $post->post_id . '']) ) {
                        ...
                        if ($cURL_status == 404) {
                            ...
                            $this->logger->logError( "Not deleting post, still exists on StatusNet, or non 404 status: "
    ...
    public function fetchInstanceUserTweets() {
        ...
        if (isset($this->user)) {
            ...
            $continue_fetching = true;
            $this->logger->logInfo("StatusNet user post count:  " . $this->user->post_count .
            ...
            && $this->user->post_count > $this->instance->total_posts_in_system && $continue_fetching) {
                ...
                $args = array();
                $count_arg =  (isset($this->statusnet_options['tweet_count_per_call']))?
                $this->statusnet_options['tweet_count_per_call']->option_value:100;
                ...
                try {
                    list($cURL_status, $statusnet_data) = $this->api->apiRequest($recent_tweets, $args);
                } catch (APICallLimitExceededException $e) {
                    break;
                }
                if ($cURL_status == 200) {
                    ...
                    //if you've got more than the StatusNet API archive limit, stop looking for more tweets
                    if ($this->instance->total_posts_in_system >= $this->api->archive_limit) {
                        $this->instance->last_page_fetched_tweets = 1;
                        $continue_fetching = false;
                        $overage_info = "StatusNet only makes ".$this->api->archive_limit.
                    ...
                    $this->user->username ." has ". $this->user->post_count." tweets according to StatusNet.";
            ...
            if (isset($this->user->username) && $this->user->username != $this->instance->network_username) {
                ...
                $post_dao = DAOFactory::getDAO('PostDAO');
                $post_dao->updateAuthorUsername($this->instance->network_user_id, 'statusnet', $this->user->username);
            }
        }
    }
    /**
    ...
    private function fetchAndAddTweetRepliedTo($tid) {
        ...
        if (isset($this->user)) {
            ...
            list($cURL_status, $statusnet_data) = $this->api->apiRequest($tweet_deets);
            $status_message = "";
            ...
            if ($cURL_status == 200) {
                ...
                foreach ($tweets as $tweet) {
                    ...
                    if ($inserted_post_key !== false) {
                        ...
                        URLProcessor::processPostURLs($tweet['post_text'], $tweet['post_id'], 'statusnet', $this->logger);
                    }
                }
            } elseif ($cURL_status == 404 || $cURL_status == 403) {
                $e = $this->api->parseError($statusnet_data);
                $ped = DAOFactory::getDAO('PostErrorDAO');
                $ped->insertError($tid, 'statusnet', $cURL_status, $e['error'], $this->user->user_id);
    ...
    /**
     * Fetch the current instance user's mentions from StatusNet and store in the database.
        ...
        if (isset($this->user)) {
            ...
            if ($this->api->available_api_calls_for_crawler > 0) {
                ...
                $continue_fetching) {
                    ...
                    try {
                        list($cURL_status, $statusnet_data) = $this->api->apiRequest($mentions, $args);
                    } catch (APICallLimitExceededException $e) {
                        break;
                    }
                    ...
                    if ($cURL_status > 200) {
                        ...
                        if (!isset($recentTweets)) {
                            $recentTweets = $post_dao->getAllPosts($this->user->user_id, 'statusnet', 100);
                        }
                        $count = 0;
                        foreach ($tweets as $tweet) {
    ...
    public function fetchRetweetsOfInstanceUser() {
        ...
        if (isset($this->user)) {
            ...
            if ($this->api->available && $this->api->available_api_calls_for_crawler > 0) {
                ...
                list($cURL_status, $statusnet_data) = $this->api->apiRequest($rtsofme);
                if ($cURL_status == 200) {
                    $tweets = $this->api->parseXML($statusnet_data);
                    foreach ($tweets as $tweet) {
    ...
    private function fetchStatusRetweets($status) {
        ...
        if ($this->api->available && $this->api->available_api_calls_for_crawler > 0) {
            ...
            list($cURL_status, $statusnet_data) = $this->api->apiRequest($rts);
            if ($cURL_status == 200) {
                $tweets = $this->api->parseXML($statusnet_data);
                foreach ($tweets as $tweet) {
    ...
    private function fetchUserTimelineForRetweet($retweeted_status, $user_with_retweet) {
        ...
        if ($this->api->available && $this->api->available_api_calls_for_crawler > 0) {
            ...
            try {
                list($cURL_status, $statusnet_data) = $this->api->apiRequest($stream_with_retweet, $args);
            } catch (APICallLimitExceededException $e) {
                return;
            }
    ...
    private function fetchInstanceUserFollowersByIDs() {
        ...
        while ($this->api->available && $this->api->available_api_calls_for_crawler > 0 && $continue_fetching) {
            ...
            try {
                list($cURL_status, $statusnet_data) = $this->api->apiRequest($follower_ids, $args);
            } catch (APICallLimitExceededException $e) {
                break;
            }
            ...
            if ($cURL_status > 200) {
                ...
                $ids = $this->api->parseXML($statusnet_data);
                ...
                foreach ($ids as $id) {
                    ...
                    if ($fd->followExists($this->instance->network_user_id, $id['id'], 'statusnet')) {
                        //update it
                        if ($fd->update($this->instance->network_user_id, $id['id'], 'statusnet',
                        Utils::getURLWithParams($follower_ids, $args)))
                        $updated_follow_count = $updated_follow_count + 1;
                    } else {
                        //insert it
                        if ($fd->insert($this->instance->network_user_id, $id['id'], 'statusnet',
                        Utils::getURLWithParams($follower_ids, $args)))
            ...
            && !$this->instance->is_archive_loaded_follows) {
                ...
                if ($cURL_status > 200) {
                    ...
                    foreach ($users as $u) {
                        ...
                        // add/update follow relationship
                        if ($fd->followExists($this->instance->network_user_id, $utu->user_id, 'statusnet')) {
                            //update it
                            if ($fd->update($this->instance->network_user_id, $utu->user_id, 'statusnet',
                            Utils::getURLWithParams($follower_ids, $args)))
                            $updated_follow_count++;
                        } else {
                            //insert it
                            if ($fd->insert($this->instance->network_user_id, $utu->user_id, 'statusnet',
                            Utils::getURLWithParams($follower_ids, $args)))
    ...
    public function updateStaleGroupMemberships() {
        ...
        if (isset($this->user) && $this->api->available_api_calls_for_crawler > 0) {
            ...
            while ($this->api->available && $this->api->available_api_calls_for_crawler > 0) {
                ...
                list($cURL_status, $statusnet_data) = $this->api->apiRequest($check_group_member, $args);

                if ($cURL_status == 404) {
                    $status_message = sprintf('User, %s, no longer active on statusnet list, %s',
                    ...
                    $status_message = sprintf('Updating active member, %s, on statusnet list, %s',
                    $this->instance->network_username, $stale_membership->group_name);
                    $this->logger->logInfo($status_message, __METHOD__.','.__LINE__);
                    $group_member_dao->update($this->instance->network_user_id, $stale_membership->group_id, 'statusnet');
                    $updated_group_member_count++;
                }
            }
            if ($updated_group_member_count > 0) {
                ...
                $group_member_count_dao->updateCount($this->instance->network_user_id, 'statusnet');
            }
        }
    }
    ...
    public function fetchInstanceUserGroups() {
        ...
        if (isset($this->user) && $this->api->available_api_calls_for_crawler > 0) {
            ...
            while ($this->api->available && $this->api->available_api_calls_for_crawler > 0 && $continue_fetching) {
                ...
                try {
                    list($cURL_status, $statusnet_data) = $this->api->apiRequest($groups, $args);
                } catch (APICallLimitExceededException $e) {
                    break;
                }
                ...
                if ($cURL_status > 200) {
                    ...
                    $status_message = "Parsing XML. ";
                    $status_message .= "Cursor ".$next_cursor.":";
                    $groups = $this->api->parseXML($statusnet_data);
                    $next_cursor = $this->api->getNextCursor();
                    if (!isset($next_cursor)) {
                        $continue_fetching = false;
                    }
    ...
    public function fetchInstanceUserFriends() {
        ...
        if (isset($this->user)) {
            ...
            if ($this->instance->total_friends_in_system < $this->user->friend_count) {
                ...
                $this->user->friend_count." friends according to StatusNet; Friend archive is not loaded",
                __METHOD__.','.__LINE__);
            } else {
            ...
            && !$this->instance->is_archive_loaded_friends) {
                ...
                try {
                    list($cURL_status, $statusnet_data) = $this->api->apiRequest($friend_ids, $args);
                } catch (APICallLimitExceededException $e) {
                    break;
                }
                ...
                if ($cURL_status > 200) {
                    ...
                    foreach ($users as $u) {
                        ...
                        // add/update follow relationship
                        if ($fd->followExists($utu->user_id, $this->instance->network_user_id, 'statusnet')) {
                            //update it
                            if ($fd->update($utu->user_id, $this->instance->network_user_id, 'statusnet',
                            Utils::getURLWithParams($friend_ids, $args)))
                            $updated_follow_count++;
                        } else {
                            //insert it
                            if ($fd->insert($utu->user_id, $this->instance->network_user_id, 'statusnet',
                            Utils::getURLWithParams($friend_ids, $args)))
                            $inserted_follow_count++;
                        }
                    }
    ...
    public function fetchFriendTweetsAndFriends() {
        ...
        if (isset($this->user)) {
            ...
            $continue_fetching = true;
            while ($this->api->available && $this->api->available_api_calls_for_crawler > 0 && $continue_fetching) {
                $stale_friend = $fd->getStalestFriend($this->user->user_id, 'statusnet');
                if ($stale_friend != null) {
                    ...
                    try {
                        list($cURL_status, $statusnet_data) = $this->api->apiRequest($stale_friend_tweets, $args);
                    } catch (APICallLimitExceededException $e) {
                        break;
                    }
                    ...
                    if ($cURL_status == 200) {
                        ...
                        (isset($e['error']))?$e['error']:$statusnet_data,
                        $this->user->user_id, 'statusnet');
    ...
    public function fetchUnloadedFollowerDetails() {
        ...
        if (isset($this->user)) {
            $fd = DAOFactory::getDAO('FollowDAO');
            $strays = $fd->getUnloadedFollowerDetails($this->user->user_id, 'statusnet');
    ...
    private function fetchUserFriendsByIDs($uid, $fd) {
        ...
        while ($this->api->available && $this->api->available_api_calls_for_crawler > 0 && $continue_fetching) {
            ...
            if ($cURL_status > 200) {
                ...
                foreach ($ids as $id) {
                    // add/update follow relationship
                    if ($fd->followExists($id['id'], $uid, 'statusnet')) {
                        //update it
                        if ($fd->update($id['id'], $uid, 'statusnet', Utils::getURLWithParams($friend_ids, $args))) {
                            $updated_follow_count++;
                        }
                    } else {
                        //insert it
                        if ($fd->insert($id['id'], $uid, 'statusnet', Utils::getURLWithParams($friend_ids, $args))) {
                            $inserted_follow_count++;
                        }
                    }
                }
    ...
    /**
     * Fetch user from StatusNet and add to DB
     * @param int $fid
    ...
    private function fetchAndAddUser($fid, $source) {
        ...
        list($cURL_status, $statusnet_data) = $this->api->apiRequest($u_deets);

        if ($cURL_status == 200) {
            $user_arr = $this->api->parseXML($statusnet_data);
            if (isset($user_arr[0])) {
            ...
            $e = $this->api->parseError($statusnet_data);
            $ued = DAOFactory::getDAO('UserErrorDAO');
            $ued->insertError($fid, $cURL_status, $e['error'], $this->user->user_id, 'statusnet');
    ...
    public function cleanUpFollows() {
        ...
        $num_errors < $num_allowed_errors) {
            $oldfollow = $fd->getOldestFollow('statusnet');
            if ($oldfollow != null) {
                ...
                try {
                    list($cURL_status, $statusnet_data) = $this->api->apiRequest($friendship_call, $args);
                } catch (APICallLimitExceededException $e) {
                    break;
                }

                if ($cURL_status == 200) {
                    $friendship = $this->api->parseXML($statusnet_data);
                    if ($friendship['source_follows_target'] == 'true') {
                        $fd->update($oldfollow["followee_id"], $oldfollow["follower_id"], 'statusnet',
                        Utils::getURLWithParams($friendship_call, $args));
                    } else {
                        $fd->deactivate($oldfollow["followee_id"], $oldfollow["follower_id"], 'statusnet',
                        Utils::getURLWithParams($friendship_call, $args));
                    }
                    if ($friendship['target_follows_source'] == 'true') {
                        $fd->update($oldfollow["follower_id"], $oldfollow["followee_id"], 'statusnet',
                        Utils::getURLWithParams($friendship_call, $args));
                    } else {
                        $fd->deactivate($oldfollow["follower_id"], $oldfollow["followee_id"], 'statusnet',
                        Utils::getURLWithParams($friendship_call, $args));
                    }
                } else {
     ...
     *
     * Recently, the StatusNet favorites API has developed some bugs that need to be worked around.  The comments below
     * provide more detail, but in a nutshell, these methods can not currently use information from StatusNet to
     * calculate loop termination (so a bit more work may be done than necessary), and do not currently remove un-fav'd
     * tweets from the database.  Hopefully these API issues will be fixed by StatusNet in future.
     */
    public function fetchInstanceFavorites() {
        ...
        // the owner favs count, from statusnet, is currently unreliable and may be less than the actual number of
        ...
        if ($last_page_fetched_favorites == 1) {
            ...
            // statusnet profile information is not always consistent, so ensure that this value is not negative
    ...
    $new_favs_to_add, $last_fav_id, $last_page_fetched_favorites, $continue) {
        ...
        list($tweets, $cURL_status, $statusnet_data) = $this->getFavsPage($mpage);
        if ($cURL_status == 200) {
            ...
            if ($older_favs_smode == false) {
                ...
                // hopefully this will be fixed again by StatusNet at some point.
                if ($mpage > $stop_page ) {
                    $continue = false;
                }
            }
            ...
            $this->logger->logInfo($statusnet_data, __METHOD__.','.__LINE__);
            $continue = false;
        }
        return array($fcount, $mpage, $older_favs_smode, $stop_page, $new_favs_to_add, $last_fav_id,
        $last_page_fetched_favorites, $continue);
     ...
     * largest page number (oldest favs), calculated based on the # of favs for the user as reported by statusnet,
    ...
    private function archivingFavsFetch ($fcount, $last_fav_id, $last_page_fetched_favorites, $continue) {
        ...
        // __METHOD__.','.__LINE__);
        list($tweets, $cURL_status, $statusnet_data) = $this->getFavsPage($last_page_fetched_favorites - 1);
     ...
     * @param int $page
     * @return array ($tweets, $cURL_status, $statusnet_data)
     */
    private function getFavsPage($page) {
        ...
        $args = array();
        $args["page"] = $page;
        list($cURL_status, $statusnet_data) = $this->api->apiRequest($favs_call, $args);
        if ($cURL_status == 200) {
            // Parse the XML file
            $tweets = $this->api->parseXML($statusnet_data);
     ...
     * the database, as well as favs that were added to the db but are no longer returned by StatusNet's API.
     * However, that latter calculation, for un-fav'd tweets, is currently not reliable due to a bug on StatusNet's end,
     ...
     * returned from StatusNet (that is, it is currently not returning some actually-favorited tweets in a given range).
     * So, we may miss some older tweets that were in fact favorited, until StatusNet fixes this.
    ...
    public function cleanUpMissedFavsUnFavs() {
        ...
        $this->api->available_api_calls_for_crawler ) {
            ...
            try {
                list($tweets, $cURL_status, $statusnet_data) = $this->getFavsPage($page);
            } catch (APICallLimitExceededException $e) {
                break;
            }
            if ($cURL_status != 200 || $tweets == -1) {
                // todo - handle more informatively
                $this->logger->logInfo("in cleanUpMissedFavsUnFavs, error with: $statusnet_data",
                __METHOD__.','.__LINE__);
            ...
            foreach ($tweets as $fav) {
                $fav['network'] = 'statusnet';
                // check whether the tweet is in the db-- if not, add it.
                if ($fpd->addFavorite($this->user->user_id, $fav) > 0) {
                    URLProcessor::processPostURLs($fav['post_text'], $fav['post_id'], 'statusnet', $this->logger);
            ...
            // favorited. This part of the method is currently disabled due to issues with the StatusNet API, which
            ...
            $fposts = $fpd->getAllFavoritePostsUpperBound($this->user->user_id, 'statusnet', $pagesize, $max_tweet + 1);
            foreach ($fposts as $old_fav) {
                ...
                if (!$found) { // if it's not there...
                    // 14/10 arghh -- StatusNet is suddenly (temporarily?) not returning all fav'd tweets in a
                    // sequence.
                    ...
                    $this->logger->logInfo("StatusNet claims tweet not still favorited, but this is currently ".

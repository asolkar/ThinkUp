...
/**
 *
 * ThinkUp/webapp/plugins/statusnet/model/class.CrawlerStatusNetAPIAccessorOAuth.php
 *
 ...
 *
 *
 * Crawler StatusNetAPI Accessor, via OAuth
 *
 ...
 *
 */
class CrawlerStatusNetAPIAccessorOAuth extends StatusNetAPIAccessorOAuth {
    /**
     *
     * @var int
     */
     ...
     * @var int
     */
    var $available_api_calls_for_statusnet = null;
    /**
     *
     * @var int
     */
    ...
    /**
     * A list of StatusNet API error codes and their explanations.
     * @var array
     */
    private $error_codes = array(
         ...
         '500' => 'Something is broken on StatusNet\'s end.',
         '502' => 'StatusNet is down or being upgraded.',
         '503' => 'The StatusNet servers are up, but overloaded with requests. Try again later.'
         );
          ...
          * @param int $num_statusnet_errors
          * @param int $max_api_calls_per_crawl
          * @return CrawlerStatusNetAPIAccessorOAuth
          */
         public function __construct($oauth_token, $oauth_token_secret, $oauth_consumer_key, $oauth_consumer_secret,
         $api_calls_to_leave_unmade_per_minute, $archive_limit, $num_statusnet_errors, $max_api_calls_per_crawl) {
         ...
         public function init() {
             ...
             list($cURL_status, $statusnet_data) = $this->apiRequest($account_status);
             ...
             if ($cURL_status > 200) {
                 ...
                 $status = $this->parseXML($statusnet_data);

                 if (isset($status['remaining-hits']) && isset($status['hourly-limit']) &&
                 isset($status['reset-time'])){
                     $this->available_api_calls_for_statusnet = $status['remaining-hits'];//get this from API
                 ...
                 $this->available_api_calls_for_crawler = $this->available_api_calls_for_statusnet -
                 round($this->api_calls_to_leave_unmade);
                 //Enforce configurable ceiling for whitelisted StatusNet accounts
         ...
         public function apiRequest($url, $args = array(), $auth = true, $suppress_404_error = false) {
             ...
             if ($auth) {
                 while ($attempts <= $this->num_retries && $continue) {
                     ...
                     $this->available_api_calls_for_statusnet = $this->available_api_calls_for_statusnet - 1;
                     ...
                     if ($url != "https://api.statusnet.com/1/account/rate_limit_status.xml") {
         ...
         public function getStatus($calling_function = null) {
             $status = $this->available_api_calls_for_statusnet." of ".$this->api_hourly_limit." StatusNet API calls ".
         ...
         /**
          * Translates a StatusNet API code to its corresponding explanation, as described in this link:
          * http://dev.statusnet.com/pages/responses_errors
          *
          ...
          *
          * @return array key => pairs of error codes that can be returned from the StatusNet API.
          */
         public function getStatusNetErrorCodes() {

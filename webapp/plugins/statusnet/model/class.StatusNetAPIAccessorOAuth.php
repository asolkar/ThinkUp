...
/**
 *
 * ThinkUp/webapp/plugins/statusnet/model/class.StatusNetAPIAccessorOAuth.php
 *
 ...
 *
 *
 * StatusNet API Accessor
 * Accesses the StatusNet.com API via OAuth authentication.
 *
...
class StatusNetAPIAccessorOAuth {
    /**
     * @var boolean
     */
    var $available = true;
     ...
     * down activity for whitelisted StatusNet accounts which get 20k calls per hour.
     ...
     * @return StatusNetAPIAccessorOAuth
     */
    public function __construct($oauth_access_token, $oauth_access_token_secret, $oauth_consumer_key,
    $oauth_consumer_secret, $num_statusnet_errors, $max_api_calls_per_crawl, $log=true) {
        $this->$oauth_access_token = $oauth_access_token;
        $this->$oauth_access_token_secret = $oauth_access_token_secret;
        $this->log = $log;

        $this->to = new StatusNetOAuthThinkUp($oauth_consumer_key, $oauth_consumer_secret, $this->$oauth_access_token,
        $this->$oauth_access_token_secret);
        $this->cURL_source = $this->prepAPI();

        $logger = Logger::getInstance();
        $te = (int) $num_statusnet_errors;
    ...
    /**
     * Verify OAuth StatusNet credentials.
     * @return mixed -1 if not authorized; array of user data if authorized
     */
    public function verifyCredentials() {
        $auth = $this->cURL_source['credentials'];
        list($cURL_status, $statusnet_data) = $this->apiRequestFromWebapp($auth);
        if ($cURL_status == 200) {
            $user = $this->parseXML($statusnet_data);
            return $user[0];
        } else {
            return - 1;
        }
    ...
    /**
     * Define how to access the StatusNet API.
     * @return array URLs by API call.
     */
    protected function prepAPI() {
        // Define how to access StatusNet API
        $api_domain = 'https://identi.ca/api';
        $api_format = 'xml';
        $search_domain = 'http://search.statusnet.com';
        $search_format = 'json';
    ...
    public function parseJSON($data) {
        ...
        foreach ($pj->results as $p) {
            ...
            'in_reply_to_post_id'=>'', 'author_fullname'=>'', 'full_name'=>'', 'source'=>'statusnet',
            'location'=>'', 'url'=>'',
    ...
    /**
     * Parse XML data returned from StatusNet.
     * @param str $data
    ...
    private function parsePostXML($post) {
        ...
        if (isset($post->retweet_count) && !isset($post->retweeted_status)) {
            ...
            // this field holds the reported native rt count from statusnet

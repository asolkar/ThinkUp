...
/**
 *
 * ThinkUp/webapp/plugins/statusnet/controller/class.StatusNetPluginConfigurationController.php
 *
 ...
 *
 *
 * StatusNet Plugin Configuration Controller
 *
 ...
 *
 */
class StatusNetPluginConfigurationController extends PluginConfigurationController {
    /**
     *
     * @var Owner
     */
    ...
    public function authControl() {
        ...
        $this->setViewTemplate(THINKUP_WEBAPP_PATH.'plugins/statusnet/view/statusnet.account.index.tpl');
        $this->view_mgr->addHelp('statusnet', 'userguide/settings/plugins/statusnet');
        ...
        $num_statusnet_errors = $this->getPluginOption('num_statusnet_errors');
        $max_api_calls_per_crawl = $this->getPluginOption('max_api_calls_per_crawl');
        //Add public user instance
        if (isset($_GET['statusnet_username'])) { // if form was submitted
            $logger = Logger::getInstance();

            $api = new StatusNetAPIAccessorOAuth('NOAUTH', 'NOAUTH', $oauth_consumer_key, $oauth_consumer_secret,
            $num_statusnet_errors, $max_api_calls_per_crawl);
            $api_call = str_replace("[id]", $_GET['statusnet_username'], $api->cURL_source['show_user']);
            ...
            if ($cURL_status == 200) {
                ...
                if (isset($user) && $user["is_protected"] == 'false') {
                    ...
                    $i = $id->getByUsernameOnNetwork($_GET['statusnet_username'], 'statusnet');
                    ...
                    if (isset($i)) { //Instance exists
                        ...
                        $i = $id->getByUsernameOnNetwork($user["user_name"], 'statusnet');
                        $oid->insert($this->owner->id, $i->id, '', '');
                    }
                    $this->addSuccessMessage($_GET['statusnet_username']." has been added to ThinkUp.");

                    $this->addSuccessMessage("Added ".$_GET['statusnet_username']." to ThinkUp.");
                } else { // if not, return error
                    $this->addErrorMessage($_GET['statusnet_username'].
                    " is a private StatusNet account; ThinkUp cannot track it without authorization.");
                }
            } else {
                $this->addErrorMessage($_GET['statusnet_username']." is not a valid StatusNet username.");
            }
        }

        $plugin = new StatusNetPlugin();
        if ($plugin->isConfigured()) {
            $to = new StatusNetOAuth($oauth_consumer_key, $oauth_consumer_secret);
            /* Request tokens from statusnet */
            $tok = $to->getRequestToken('http://thinkupdev.mahesha.com/plugins/statusnet/auth.php'); //FIXME: MMA: parameterize me
            ...
            || (isset($_SESSION["MODE"]) && $_SESSION["MODE"] == "TESTS") || getenv("MODE")=="TESTS") { //testing
                ...
                $owner_instances = $id->getByOwnerAndNetwork($this->owner, 'statusnet');
                ...
                "Unable to obtain OAuth tokens from StatusNet. Please double-check the consumer key and secret ".
                "are correct.", "setup");
                $oauthorize_link = '';
    ...
    private function addOptionForm() {
        ...
        '[<a href="http://dev.statusnet.com/pages/every_developer" title="StatusNet still maintains a database '.
        ...
        $num_statusnet_errors_label = 'Total API Errors to Tolerate';
        $num_statusnet_errors = array('name' => 'num_statusnet_errors', 'label' => $num_statusnet_errors_label,
        'default_value' => '5', 'advanced'=>true, 'size'=>3);
        $this->addPluginOption(self::FORM_TEXT_ELEMENT, $num_statusnet_errors);

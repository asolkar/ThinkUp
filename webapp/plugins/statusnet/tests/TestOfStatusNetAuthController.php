...
/**
 *
 * ThinkUp/webapp/plugins/statusnet/tests/TestOfStatusNetAuthController.php
 *
 ...
 *
 * Test of StatusNetAuthController
 *
...
require_once THINKUP_ROOT_PATH.'webapp/plugins/statusnet/controller/class.StatusNetAuthController.php';

class TestOfStatusNetAuthController extends ThinkUpUnitTestCase {
    ...
    public function testConstructor() {
        $controller = new StatusNetAuthController(true);
    ...
    public function testLoggedInAllParams() {
        ...
        'option_name'=>'num_statusnet_errors', 'option_value'=>'5'));
        ...
        //sleep(100);
        $v_mgr = $controller->getViewManager();
        $this->assertEqual('Success! dougw on StatusNet has been added to ThinkUp!',
    ...
    public function testLoggedInAllParamsServiceUserExists() {
        ...
        'network_username'=>'dougw', 'network'=>'statusnet'));
        $builders[] = FixtureBuilder::build('instances_statusnet', array('last_page_fetched_replies'=>1));
        ...
        $v_mgr = $controller->getViewManager();
        $this->debug($results);
        $this->assertEqual('dougw on StatusNet is already set up in ThinkUp! To add a different StatusNet account, log '.
        'out of StatusNet.com in your browser and authorize ThinkUp again.', $v_mgr->getTemplateDataItem('success_msg'));
    }
}

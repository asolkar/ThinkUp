...
/**
 *
 * ThinkUp/webapp/plugins/statusnet/tests/TestOfStatusNetCrawler.php
 *
 ...
 *
 * Test of StatusNetCrawler
 *
 * @TODO Test the rest of the StatusNetCrawler methods
...
require_once THINKUP_ROOT_PATH.'webapp/plugins/statusnet/model/class.StatusNetAPIAccessorOAuth.php';
require_once THINKUP_ROOT_PATH.'webapp/plugins/statusnet/model/class.CrawlerStatusNetAPIAccessorOAuth.php';
require_once THINKUP_ROOT_PATH.'webapp/plugins/statusnet/model/class.StatusNetCrawler.php';
require_once THINKUP_ROOT_PATH.'webapp/plugins/statusnet/model/class.StatusNetOAuthThinkUp.php';
require_once THINKUP_ROOT_PATH.'webapp/plugins/statusnet/model/class.RetweetDetector.php';
require_once THINKUP_ROOT_PATH.'webapp/plugins/statusnet/model/class.StatusNetInstance.php';
require_once THINKUP_ROOT_PATH.'webapp/plugins/statusnet/model/class.StatusNetInstanceMySQLDAO.php';

class TestOfStatusNetCrawler extends ThinkUpUnitTestCase {
    /**
     * @var CrawlerStatusNetAPIAccessorOAuth API accessor object
     */
    var $api;
    /**
     *
    ...
    public function setUp() {
        ...
        'full_name'=>'Anil Dash', 'last_updated'=>'2007-01-01 20:34:13', 'network'=>'statusnet', 'is_protected'=>0,
        'last_post_id'=>''));

        $this->builders[] = FixtureBuilder::build('users', array('user_id'=>'930061', 'user_name'=>'ginatrapani',
        'full_name'=>'Gina Trapani', 'last_updated'=>'2007-01-01 20:34:13', 'network'=>'statusnet', 'is_protected'=>0,
        'last_post_id'=>''));
        ...
        'group_name'=>'@userx/anotherlist', 'network' => 'statusnet', 'is_active'=>1));

        // stale group membership
        $this->builders[] = FixtureBuilder::build('group_members', array('member_user_id'=>'36823',
        'group_id'=>'19994710', 'is_active' => 1, 'network'=>'statusnet', 'last_seen' => '-3d'));
    }
    ...
    private function setUpInstanceUserAnilDash() {
        ...
        'is_active'=>'0', 'network'=>'statusnet', 'last_favorite_id' => '0', 'last_unfav_page_checked' => '0',
        ...
        $this->instance = new StatusNetInstance($r);

        $this->api = new CrawlerStatusNetAPIAccessorOAuth('111', '222', 'fake_key', 'fake_secret', 2,
        1234, 5, 350);
    ...
    private function setUpInstanceUserAnilDashUsernameChange() {
        ...
        'post_text'=>'This is a great post', 'network'=>'statusnet','in_rt_of_user_id' => null,
        'in_reply_to_post_id'=>null, 'in_retweet_of_post_id'=>null, 'is_geo_encoded'=>0));
        $builders[] = FixtureBuilder::build('users', array('user_id'=>'36824', 'network'=>'statusnet'));
        return $builders;
    }
    ...
    private function setUpInstanceUserPrivateMcprivate() {
        ...
        'full_name'=>'Private McPrivate', 'last_updated'=>'2007-01-01 20:34:13', 'network'=>'statusnet',
    ...
    private function setUpInstanceUserGinaTrapani() {
        ...
        'avg_replies_per_day'=>'2', 'is_public'=>'0', 'is_active'=>'0', 'network'=>'statusnet',
        ...
        $this->api = new CrawlerStatusNetAPIAccessorOAuth('111', '222', 'fake_key', 'fake_secret', 2, 1234, 5, 350);
        $this->api->available = true;
        $this->api->available_api_calls_for_crawler = 20;
        $this->instance->is_archive_loaded_follows = true;
    }
    ...
    private function setUpInstanceUserAmygdala() {
        $instd = DAOFactory::getDAO('StatusNetInstanceDAO');
        $iid = $instd->insert('2768241', 'amygdala', 'statusnet');
        $this->instance = $instd->getByUsernameOnNetwork("amygdala", "statusnet");

        $this->api = new CrawlerStatusNetAPIAccessorOAuth('111', '222', 'fake_key', 'fake_secret',2, 1234,
        5, 350);
        $this->api->available = true;
        $this->api->available_api_calls_for_crawler = 20;
    }
    ...
    public function testConstructor() {
        ...
        $statusnet_crawler = new StatusNetCrawler($this->instance, $this->api);

        $this->assertTrue($statusnet_crawler != null);
    }
    ...
    public function testFetchInstanceUserInfo() {
        ...
        $statusnet_crawler->fetchInstanceUserInfo();

        $user_dao = DAOFactory::getDAO('UserDAO');
        $user = $user_dao->getDetails('36823', 'statusnet');
    ...
    public function testFetchInstanceUserTweets() {
        ...
        $statusnet_crawler = new StatusNetCrawler($this->instance, $this->api);
        $statusnet_crawler->fetchInstanceUserInfo();
        $statusnet_crawler->fetchInstanceUserTweets();

        //Test post with location has location set
        $post_dao = DAOFactory::getDAO('PostDAO');
        $this->assertTrue($post_dao->isPostInDB('15680112737', 'statusnet'));

        $post = $post_dao->getPost('15680112737', 'statusnet');
        ...
        $post = $post_dao->getPost('15660552927', 'statusnet');
        $this->assertEqual($post->location, "NYC: 40.739069,-73.987082");
    ...
    public function testFetchInstanceUserTweetsEscapeHTML() {
        ...
        //Test post with location has location set
        $post_dao = DAOFactory::getDAO('PostDAO');
        $this->assertTrue($post_dao->isPostInDB('15660373190', 'statusnet'));
        $post = $post_dao->getPost('15660373190', 'statusnet');
    ...
    public function testFetchInstanceUserTweetsBudgeted() {
        ...
        $this->assertTrue($pdao->isPostInDB(15660373190, 'statusnet'));
        $post = $pdao->getPost(15660373190, 'statusnet');
        $this->assertEqual($post->post_text, "@nicknotned NYC isn't a rival, it's just a better evolution of the " .
        "concept of a locale where innovation happens. > & <");
    ...
    public function testFetchInstanceUserTweetsUsernameChange() {
        ...
        'network_username'=>'anildash2', 'network'=>'statusnet', 'network_viewer_id'=>15,
        'crawler_last_run'=>'2010-01-01 12:00:01', 'is_active'=>1));

        $post_dao = DAOFactory::getDAO('PostDAO');

        // old post before crawl
        $post = $post_dao->getPost(1, 'statusnet');
        ...
        $post = $post_dao->getPost('15660310954', 'statusnet');
        $this->assertEqual($post->author_username, "anildash2");
    ...
    public function testDeletedTweet() {
        ...
        'network_username'=>'anildash', 'network'=>'statusnet', 'network_viewer_id'=>'36825',
        'crawler_last_run'=>'2010-01-01 12:00:01', 'is_active'=>1));

        $builders[] = FixtureBuilder::build('users', array('user_id'=>'36825', 'network'=>'statusnet'));
        ...
        // should be deleted, not  found on statusnet...
        $post_dao = DAOFactory::getDAO('PostDAO');
        $this->assertNull($post_dao->getPost('12345', 'statusnet'));

        // found on statusnet, so don't delete
        $post_dao = DAOFactory::getDAO('PostDAO');
        $this->assertNotNull($post_dao->getPost('123456', 'statusnet'));
    }
    ...
    public function testFetchPrivateInstanceUserTweets() {
        ...
        $post_dao = DAOFactory::getDAO('PostDAO');
        $this->assertTrue($post_dao->isPostInDB('14846078418', 'statusnet'));

        $post = $post_dao->getPost('14846078418', 'statusnet');
    ...
    public function testFetchInstanceUserTweetsRetweets() {
        ...
        $user_builder = FixtureBuilder::build('users', array('user_id'=>'2768241', 'network'=>'statusnet'));
        ...
        $post_dao = DAOFactory::getDAO('PostDAO');
        $post = $post_dao->getPost('13708601491193856', 'statusnet');
        ...
        $retweets = $post_dao->getRetweetsOfPost('13708601491193856', 'statusnet', true);
    ...
    public function testFetchInstanceUserTweetsRetweetsBudget() {
        ...
        $statusnet_crawler->fetchInstanceUserMentions();
        // old-style RT
        $post = $pdao->getPost('8957053141778432', 'statusnet');
    ...
    public function testFetchSearchResults() {
        ...
        $statusnet_crawler->fetchInstanceUserInfo();
        $statusnet_crawler->fetchSearchResults('@whitehouse');
        $post_dao = DAOFactory::getDAO('PostDAO');
        $this->assertTrue($post_dao->isPostInDB('11837263794', 'statusnet'));

        $post = $post_dao->getPost('11837263794', 'statusnet');
    ...
    public function testFetchSearchResultsBudget() {
        ...
        $statusnet_crawler->fetchSearchResults('@whitehouse');
        $pdao = DAOFactory::getDAO('PostDAO');
        $this->assertFalse($pdao->isPostInDB('11837263794', 'statusnet'));
    ...
    public function testFetchInstanceUserFollowers() {
        ...
        $statusnet_crawler->fetchInstanceUserFollowers();
        $follow_dao = DAOFactory::getDAO('FollowDAO');
        $this->assertTrue($follow_dao->followExists('36823', '119950880', 'statusnet'), 'new follow exists');

        $user_dao = DAOFactory::getDAO('UserDAO');
        $updated_user = $user_dao->getUserByName('meatballhat', 'statusnet');
    ...
    public function testFetchInstanceUserGroups() {
        ...
        $statusnet_crawler->fetchInstanceUserGroups();
        $group_dao = DAOFactory::getDAO('GroupDAO');
        $this->assertTrue($group_dao->isGroupInStorage($group = '1234566', 'statusnet'), 'group does not exist');

        $group_member_dao = DAOFactory::getDAO('GroupMemberDAO');
        $this->assertTrue($group_member_dao->isGroupMemberInStorage($user = '36823', $group = '1234566', 'statusnet'),
        'group member does not exist');
        $this->assertFalse($group_member_dao->isGroupMemberInStorage($user = '930061', $group = '1234566', 'statusnet'),
        'group member exists');

        $group_membership_count_dao = DAOFactory::getDAO('GroupMembershipCountDAO');
        $history = $group_membership_count_dao->getHistory($user = '36823', 'statusnet', 'DAYS');
    ...
    public function testFetchInstanceUserGroupsBudget() {
        ...
        $statusnet_crawler->fetchInstanceUserGroups();
        $group_dao = DAOFactory::getDAO('GroupDAO');
        $this->assertFalse($group_dao->isGroupInStorage($group = '1234566', 'statusnet'), 'group does not exist');

        $group_member_dao = DAOFactory::getDAO('GroupMemberDAO');
        $this->assertFalse($group_member_dao->isGroupMemberInStorage($user = '36823', $group = '1234566', 'statusnet'),
    ...
    public function testUpdateStaleGroupMemberships() {
        ...
        $group_member_dao = DAOFactory::getDAO('GroupMemberDAO');
        $this->assertTrue($group_member_dao->isGroupMemberInStorage($user = '36823', $group = '19994710', 'statusnet',
        $active = true), 'group member does not exist');

        $statusnet_crawler->updateStaleGroupMemberships();
        $this->assertFalse($group_member_dao->isGroupMemberInStorage($user = '36823', $group = '19994710', 'statusnet',
    ...
    public function testFetchInstanceUserFriends() {
        ...
        $statusnet_crawler->fetchInstanceUserFriends();
        $follow_dao = DAOFactory::getDAO('FollowDAO');
        $this->assertTrue($follow_dao->followExists('14834340', '36823', 'statusnet'), 'new friend exists');

        $user_dao = DAOFactory::getDAO('UserDAO');
        $updated_user = $user_dao->getUserByName('jayrosen_nyu', 'statusnet');
    ...
    public function testFetchInstanceUserFriendsBudget() {
        ...
        $statusnet_crawler->fetchInstanceUserFriends();
        $follow_dao = DAOFactory::getDAO('FollowDAO');
        $this->assertFalse($follow_dao->followExists(14834340, 36823, 'statusnet'), 'new friend doesn\'t exists');
    ...
    public function testFetchInstanceUserFriendsByIds() {
        ...
        $statusnet_crawler->fetchFriendTweetsAndFriends();
        $follow_dao = DAOFactory::getDAO('FollowDAO');
        $this->assertTrue($follow_dao->followExists('14834340', '930061', 'statusnet'), 'ginatrapani friend loaded');
    }
    ...
    public function testFetchInstanceUserFriendsByIdsBudget() {
        ...
        $statusnet_crawler->fetchFriendTweetsAndFriends();
        $follow_dao = DAOFactory::getDAO('FollowDAO');
        $this->assertFalse($follow_dao->followExists(14834340, 930061, 'statusnet'), 'ginatrapani notfriend loaded');
    }
    ...
    public function testFetchInstanceUserFollowersByIds() {
        ...
        $statusnet_crawler->fetchInstanceUserFollowers();
        $follow_dao = DAOFactory::getDAO('FollowDAO');
        $this->assertTrue($follow_dao->followExists('36823', '114811186', 'statusnet'), 'new follow exists');
    }
    ...
    public function testFetchInstanceUserFollowersByIdsBudget() {
        ...
        $statusnet_crawler->fetchInstanceUserFollowers();
        $follow_dao = DAOFactory::getDAO('FollowDAO');
        $this->assertFalse($follow_dao->followExists(36823, 114811186, 'statusnet'), 'new does not exists');
    }
    ...
    public function testFetchRetweetsOfInstanceuser() {
        ...
        $post_dao = DAOFactory::getDAO('PostDAO');
        $statusnet_crawler->fetchRetweetsOfInstanceUser();
        $post = $post_dao->getPost('14947487415', 'statusnet');
        ...
        $retweets = $post_dao->getRetweetsOfPost('14947487415', 'statusnet', true);
        ...
        $post = $post_dao->getPost('12722783896', 'statusnet');
        $rts2 = $post_dao->getRetweetsOfPost('12722783896', 'statusnet', true);
    ...
    public function testFetchStrayRepliedToTweets() {
        ...
        $statusnet_crawler->fetchInstanceUserTweets();
        $post_dao = DAOFactory::getDAO('PostDAO');
        $tweets = $post_dao->getAllPostsByUsername('anildash', 'statusnet');

        $statusnet_crawler->fetchStrayRepliedToTweets();
        $post = $post_dao->getPost('15752814831', 'statusnet');
    ...
    public function testFetchStrayRepliedToTweetsBudget() {
        ...
        $statusnet_crawler->fetchInstanceUserTweets();
        $pdao = DAOFactory::getDAO('PostDAO');
        $tweets = $pdao->getAllPostsByUsername('anildash', 'statusnet');

        $statusnet_crawler->fetchStrayRepliedToTweets();
        $post = $pdao->getPost(15752814831, 'statusnet');
        $this->assertTrue(isset($post));
    ...
    public function testFetchFavoritesOfInstanceuser() {
        ...
        $this->api->to->setDataPath('webapp/plugins/statusnet/tests/testdata/favs_tests/favs_stage1/');
        $statusnet_crawler = new StatusNetCrawler($this->instance, $this->api);
        $statusnet_crawler->fetchInstanceUserInfo();
        $statusnet_crawler->fetchInstanceFavorites();
        // Save instance
        $instance_dao = DAOFactory::getDAO('StatusNetInstanceDAO');
        if (isset($statusnet_crawler->user)) {
            $instance_dao->save($this->instance, $statusnet_crawler->user->post_count, $this->logger);
        }
        $this->instance = $instance_dao->getByUsernameOnNetwork("amygdala", "statusnet");
        ...
        $this->api->to->setDataPath('webapp/plugins/statusnet/tests/testdata/favs_tests/favs_stage2/');
        $statusnet_crawler = new StatusNetCrawler($this->instance, $this->api);
        $statusnet_crawler->fetchInstanceUserInfo();
        $statusnet_crawler->fetchInstanceFavorites();
        // Save instance
        ...
        $this->api->available_api_calls_for_crawler = 4;
        $this->api->to->setDataPath('webapp/plugins/statusnet/tests/testdata/favs_tests/favs_stage3/');
        $statusnet_crawler = new StatusNetCrawler($this->instance, $this->api);
        $statusnet_crawler->fetchInstanceUserInfo();
        $retval = $statusnet_crawler->fetchInstanceFavorites();
        // Save instance
        if (isset($statusnet_crawler->user)) {
            $instance_dao->save($this->instance, $statusnet_crawler->user->post_count, $this->logger);
        }
        ...
        // now test case where there are 'extra' favs being reported by statusnet,
        ...
        $this->api->available_api_calls_for_crawler = 10;
        $this->api->to->setDataPath('webapp/plugins/statusnet/tests/testdata/favs_tests/favs_stage5/');
        $statusnet_crawler = new StatusNetCrawler($this->instance, $this->api);
        $statusnet_crawler->fetchInstanceUserInfo();
        $retval = $statusnet_crawler->fetchInstanceFavorites();
        // Save instance
        ...
        $post = $post_dao->getPost('25138632577', 'statusnet');
        ...
        $post = $post_dao->getPost('25598018110', 'statusnet');
        $this->assertIsA($post->links[0], "Link");
    ...
    public function testFetchFavoritesOfInstanceuserBadResponse() {
        ...
        self::setUpInstanceUserAmygdala();
        $this->api->available_api_calls_for_crawler = 10;
        $this->api->to->setDataPath('webapp/plugins/statusnet/tests/testdata/favs_tests/favs_stage4/');
        $statusnet_crawler = new StatusNetCrawler($this->instance, $this->api);
        $statusnet_crawler->fetchInstanceUserInfo();
        $retval = $statusnet_crawler->fetchInstanceFavorites();
        // Save instance
    ...
    public function testNoFavorites() {
        ...
        self::setUpInstanceUserAmygdala();
        $this->api->available_api_calls_for_crawler = 10;
        $this->api->to->setDataPath('webapp/plugins/statusnet/tests/testdata/favs_tests/favs_stage7/');
        $statusnet_crawler = new StatusNetCrawler($this->instance, $this->api);
        $statusnet_crawler->fetchInstanceUserInfo();
        $retval = $statusnet_crawler->fetchInstanceFavorites();
        // Save instance
    ...
    public function testNegPageRecovery() {
        ...
        self::setUpInstanceUserAmygdala();
        $this->api->available_api_calls_for_crawler = 10;
        $this->api->to->setDataPath('webapp/plugins/statusnet/tests/testdata/favs_tests/favs_stage8/');
    ...
    public function testCleanupMissedFavs() {
        ...
        $statusnet_crawler = new StatusNetCrawler($this->instance, $this->api);
        $statusnet_crawler->fetchInstanceUserInfo();
        $retval = $statusnet_crawler->cleanUpMissedFavsUnFavs();
        $this->assertEqual($retval, true);
    ...
    public function testAddRmOldFavMaintSearch() {
        ...
        $this->api->to->setDataPath('webapp/plugins/statusnet/tests/testdata/favs_tests/favs_stage6/');
        $statusnet_crawler = new StatusNetCrawler($this->instance, $this->api);
        $statusnet_crawler->fetchInstanceUserInfo();
        ...
        // update: due to issue with StatusNetAPI, not currently removing un-favs from database
    ...
    public function testCleanUpFollows404() {
        ...
        $follow_dao = DAOFactory::getDAO('FollowDAO');
        $this->assertTrue($follow_dao->followExists('930061', '36823', 'statusnet', true), 'Active follow exists');

        $statusnet_crawler->cleanUpFollows();
        $this->assertFalse($follow_dao->followExists('930061', '36823', 'statusnet', true), 'Follow marked inactive');
    }
    ...
    public function testCleanUpFollowsBudget() {
        ...
        $statusnet_crawler->cleanUpFollows();
        $follow_dao = DAOFactory::getDAO('FollowDAO');
        $this->assertTrue($follow_dao->followExists(930061, 36823, 'statusnet', true), 'Follow not marked inactive');
    }

    public function testLoggingErrorOutput() {
        self::setUpInstanceUserGinaTrapani();
        foreach ($this->api->getStatusNetErrorCodes() as $error_code => $explanation) {
